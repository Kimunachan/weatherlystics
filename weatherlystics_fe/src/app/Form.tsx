...
const Form = ({ location, DateValue, showSecondDate, toggleSecondDate, getGeolocation }:FormProps) => {
    const [selectedTimezone, setSelectedTimezone] = useState<{ value: string, label: string }>();
  
    const { data: timezones, isLoading: isLoadingTimezones } = useQuery({
        queryKey: ["timezones"],
        queryFn: async () => {
            const response = await axios.get<string[]>("http://worldtimeapi.org/api/timezone");
            return response.data.map((timezone) => ({ value: timezone, label: timezone }));
        },
        
    });
    const handleTimezoneChange = (selectedOption:{ value: string, label: string }) => {
        ...
        <form className="form">
            <div className="form-row">
                    ...
                    <div className="form-row">
                        <label>
                            Long:
                            <input type="text" name="long" defaultValue={location?.longitude || ''} />
                        </label>
