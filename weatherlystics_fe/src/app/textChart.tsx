...
export default function textChart({ weatherData }: TextChartProps) {
            ...
            return (
                <div>
                    {data.map((chart, chartIndex) => (
                        chart.datasets && <div key={chartIndex}>
                            {chart.datasets.map((dataset, datasetIndex) => (
                                chart.labels && <div key={datasetIndex}>
                                    <h3>{dataset.label}</h3>
                                    <ul>
                                        {chart.labels.map((label: string, labelIndex: number) => (
                                            <li key={labelIndex}>
    ...
    return (
        <div className={styles.text_container}>
            {chartData && Array.isArray(chartData) ? chartData.map((data, index) => (
                <div key={index}>
                    <section data-testid={`chart_temp_${index}`} className="chart_temp">
                        {renderTextData(data.temperatureChart, "°C")}
                    </section>
                    <section data-testid={`chart_humidity_${index}`} className="chart_humidity">
                        {renderTextData(data.humidityChart, "%")}
                    </section>
                    <section data-testid={`chart_appTemp_${index}`} className="chart_appTemp">
